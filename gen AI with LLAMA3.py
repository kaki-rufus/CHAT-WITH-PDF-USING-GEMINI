...
def process_document(uploaded_file: UploadedFile) -> list[Document]:
    ...
    try:
        ...
        # Debugging: Check if docs contains any content
        if not docs:
            st.warning("No content found in the uploaded document.")
        else:
            st.write(f"Document loaded successfully. Number of pages: {len(docs)}")
        ...
        # Debugging: Check if any splits were created
        if not split_docs:
            st.warning("Document was not split into any chunks.")
        else:
        ...
        if temp_file_path and os.path.exists(temp_file_path):
            os.unlink(temp_file_path)  # Remove the temp file after processing
...
def add_to_vector_collection(all_splits: list[Document], file_name: str):
    ...
    """
    if not all_splits:
        st.warning("No documents to add to the vector collection.")
        return
    ...
    if not documents or not metadatas or not ids:
        st.warning("One or more required lists (documents, metadatas, ids) are empty.")
        return

    # Debugging: print the lengths of the lists
    st.write(f"Documents: {len(documents)}, Metadatas: {len(metadatas)}, IDs: {len(ids)}")
...
#     """Re-rank documents using a cross-encoder model for more accurate relevance scoring."""
#     relevant_text = ""
#     relevant_text_ids = []

#     encoder_model = CrossEncoder("cross-encoder/ms-marco-MiniLM-L-6-v2")

#     # Create a list where the same query is repeated for each document
#     queries = [query] * len(documents)

#     # Re-rank documents based on the query
#     ranks = encoder_model.rank(queries, documents, top_k=3)
...
if __name__ == "__main__":
    ...
    #     # Make sure that documents is a list and not a single document
    #     if documents:
    #         context = documents[0]
    #         relevant_text, relevant_text_ids = re_rank_cross_encoders(query=prompt, documents=documents)
    #         response = call_llm(context=relevant_text, prompt=prompt)
    #         st.write_stream(response)
    ...
    #     results = query_collection(prompt)
    #     context = results.get("documents")[0]
    #     # relevant_text, relevant_text_ids = re_rank_cross_encoders([doc['content'] for doc in results["documents"]])
    #     # Assuming results["documents"] is a list of strings (document contents)
    #     relevant_text, relevant_text_ids = re_rank_cross_encoders(results["documents"])
    #     response = call_llm(context=relevant_text, prompt=prompt)
    #     st.write_stream(response)
